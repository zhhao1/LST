...
class AsrDataset(data.Dataset):
    ...
    def __init__(self, tsv_root: Path, completed_ids: List[str], dataset_name: str) -> None:
    ...
    def __getitem__(self, i: int) -> Tuple[str, np.array, str]:
        ...
        # get datapoint
        if self.dataset_name == "MUSTC":
            _id, audio, tgt_text = self.data.iloc[i][["id", "audio", "src_text"]]
        else:
