...
def find_noisy_examples_zhhao(df: pd.DataFrame, asr_predictions_file: Path,
asr_wer_theshold: float, max_example_number: int) -> pd.Series:
    ...
    examples_bool = df.WER <= asr_wer_theshold
    if examples_bool.sum() > max_example_number:
        examples_bool[max_example_number:] = False
    return examples_bool
